...
source_refs:              # 可追溯来源
  - type: commit|pr|issue|test|post|paper
    ref: ""
    summary: ""
...
4. **`confirmed` 只来自两条路径**：用户明确输入，或命中已批准 fingerprint（`PositionApprovalRepository.find_active` 命中且未撤销）。
...
7. **Skill 层只产契约，持久化复用 ContentJob**：Skill 不直接写库，经 `IdeaService.create_candidate` 落为 `ContentJob`（`origin`/`generation_key`/`generator_name`/`generator_version`/`content_fingerprint`/`idea_candidate_json`）。
