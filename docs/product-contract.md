...
Finch 是一个**跨行业连接与灵感助理**：帮你发现不同领域中持续创造、分享一手经验的人，理解值得了解或交流的原因，延续对话并积累关系上下文，保存由观察、交流和实践引发的新问题、新视角与方法；公开表达（从实践与交流中形成自己的观点、写成像自己的内容）是可选出口。
...
- **召回预算**：行业开放、主题轮换，不固定 AI/Agent 或近中远配额；查询槽位用于覆盖不同实践背景，不足不凑数。
- **每日结构**：首页 3 个重点、50 人分层浏览（来自同一快照）；已有对话独立呈现；选中后建议每日深入 1 人、最多 3 位。
- **行动**：有真实贡献或具体问题才准备互动；否则先了解/试用。`next_action` 由 Python 从模式与实践依据派生，不为所有候选强行生成回复。
- **时间预算**：默认每日 10–15 分钟；建议行动须给出预计分钟，可跳过或缩小。
