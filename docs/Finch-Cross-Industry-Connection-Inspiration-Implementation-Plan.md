...
日期：2026-09-18  
状态：两轮需求与实现取舍均已确认，待实施。此次只更新计划，不代表仓库代码已修改。  
...
- [ ] 记录HEAD，读取AGENTS.md，核对本表路径和接口。
- [ ] 将D1–D10、F1–F2映射到实际函数与已有测试。
- [ ] 准备来源可追溯的语义样例：单作品实践者、陌生领域提问、未知用途、低证据、重复作品、模拟讨论。
- [ ] 更新产品契约，预定功能明确标待实施。
...
- [ ] D4单作品证据评估与重点资格端到端一致。
- [ ] D3选中后支持观察提问，亲历断言仍要求证据。
- [ ] D7首页3人及独立跟进，与浏览同快照。
- [ ] D8薄灵感保存、人物回看、周回顾；保留按需碰撞与实验。
- [ ] 更新对应Skill并进行5次真实使用，覆盖三个入口。

验收：发现一件可信作品→用户选择了解或提问→保存一条灵感→再次找到；不要求已有个人经验或实验计划。无真实使用条件时明确记录待用户验收，不能用模拟反馈代替。

### P3：优化发现质量与配置

- [ ] D5修正重复证据计分，保留基础分；回归单作品入围与评分解释。
- [ ] D6实践背景差异替换四方向/平台新颖性组合。
- [ ] D9显式主题查询组轮换，保留固定查询与GitHub登录名语义。
- [ ] 加入轻量反馈，不将“今天没时间”映射成长期排斥。
- [ ] 统一旧入口与JSON兼容视图，核对所有文档。
